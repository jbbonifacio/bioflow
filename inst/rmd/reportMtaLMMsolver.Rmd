      ...
      for(iTrait in traits){
        ...
        if(!is.null(temp.pred.box)){
          if(nrow(temp.pred.box) > 0){
            ...
            if(iType %in% c("environment", "designation", "(Intercept)")){
              print(p + labs(x = "Environmental mean") +
                      ggplot2::theme(axis.text.x = ggplot2::element_blank()))
              # print(htmltools::tagList(plotly::ggplotly(p + labs(x = "Environmental mean") +
              #         ggplot2::theme(axis.text.x = ggplot2::element_blank()))))
            } else{
              print(p + labs(x = "Environment") + ggplot2::guides(x =  guide_axis(angle = 45)))
              # print(htmltools::tagList(plotly::ggplotly(p + labs(x = "Environment")) %>% layout(xaxis = list(tickangle =-45),autosize = T, height = 800)))
            }
          }else{
            ggplot2::ggplot() + ggplot2::ggtitle("Combination not available")
    ...
    shiny::renderPlot({
      ...
      if(!is.null(temp.pred.box)){
        if(nrow(temp.pred.box) > 0){
          ...
          if(input$boxplotMtaPredsType %in% c("environment", "designation", "(Intercept)")){
            p + ggplot2::theme(axis.text.x = ggplot2::element_blank())
              # plotly::ggplotly(p + ggplot2::theme(axis.text.x = ggplot2::element_blank()))
            } else{
              p + labs(x = "Environment") + ggplot2::guides(x =  guide_axis(angle = 45))
    ...
    for(iTrait in traits){
      ...
      if(length(goodFields) > 0){
        ...
        p2 <- ggplot() + geom_histogram(data = mydataSta4[which(mydataSta4$Var1 != mydataSta4$Var2),], aes(x = Freq, fill = ..x..), binwidth=10) +
          ggplot2::scale_x_continuous(limits = c(0,ub)) +
          scale_fill_gradient2(low="firebrick",mid="gold", high="#038542", midpoint=60,
                               limit = c(0,ub), space = "Lab",
                               name="Connectivity") +
